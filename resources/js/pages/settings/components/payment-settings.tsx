...
interface PaymentSettings {
  ...
  is_moolre_enabled: boolean;
  moolre_username: string;
  moolre_public_key: string;
...
export default function PaymentSettings({ settings = {}, whatsappVariables = {}, telegramVariables = {} }: PaymentSettingsProps) {
  ...
  const { data, setData, post, processing, errors } = useForm<PaymentSettings>({
    ...
    is_moolre_enabled: settings.is_moolre_enabled === true || settings.is_moolre_enabled === '1',
  ...
  const paymentMethods = useMemo(() => {
    const methods = [
      { key: 'cod', name: t('Cash on Delivery') },
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {shouldShowMethod('cod') && (
                ...
                >
                  <p className="text-sm text-muted-foreground">
                    {t('Allow customers to pay in cash when their order is delivered. No configuration required.')}
                  </p>
                </PaymentMethodCard>
              )}
              ...
              {/* Moolre */}
              {shouldShowMethod('moolre') && (
                <PaymentMethodCard
                  title={t('Moolre')}
                  icon={<CreditCard className="h-5 w-5" />}
                  enabled={data.is_moolre_enabled}
                  onToggle={(checked) => setData('is_moolre_enabled', checked)}
                  helpUrl={PAYMENT_METHOD_HELP_URLS[PAYMENT_METHODS.MOOLRE]}
                  helpText={t("Get your Moolre API credentials from your")}
                >
                  <div className="grid grid-cols-1 gap-4">
                    <PaymentInputField
                      id="moolre_username"
                      label={t("Username")}
                      value={data.moolre_username}
                      onChange={(value) => setData('moolre_username', value)}
                      placeholder={t("Username")}
                      error={errors.moolre_username}
                    />
                    <PaymentInputField
                      id="moolre_public_key"
                      label={t("Public Key")}
                      value={data.moolre_public_key}
                      onChange={(value) => setData('moolre_public_key', value)}
                      placeholder={t("Public Key")}
                      error={errors.moolre_public_key}
                    />
                    <PaymentInputField
                      id="moolre_account_number"
                      label={t("Account Number")}
                      value={data.moolre_account_number}
                      onChange={(value) => setData('moolre_account_number', value)}
                      placeholder={t("Account Number")}
                      error={errors.moolre_account_number}
                    />
                  </div>
                </PaymentMethodCard>
              )}
