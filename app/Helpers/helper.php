...
if (!function_exists('getSetting')) {
    function getSetting($key, $default = null, $user_id = null, $store_id = null)
    {
        ...
        $paymentSettingKeys = [
            ...
            'telegram_bot_token',
            'telegram_chat_id',
            'messaging_order_variables',
            'messaging_item_variables'
        ];
...
if (!function_exists('getPaymentMethodConfig')) {
    ...
    function getPaymentMethodConfig($method, $userId = null, $storeId = null)
    {
        ...
        switch ($method) {
            ...
            case 'moolre':
                return [
                    'enabled' => isPaymentMethodEnabled('moolre', $userId, $storeId),
                    'username' => $settings['moolre_username'] ?? null,
                    'public_key' => $settings['moolre_public_key'] ?? null,
                    'account_number' => $settings['moolre_account_number'] ?? null,
                ];
...
if (!function_exists('getEnabledPaymentMethods')) {
    ...
    {
        $methods = ['cod', 'stripe', 'paypal', 'razorpay', 'mercadopago', 'paystack', 'flutterwave', 'bank', 'paytabs', 'skrill', 'coingate', 'payfast', 'tap', 'xendit', 'paytr', 'mollie', 'toyyibpay', 'cashfree', 'iyzipay', 'benefit', 'ozow', 'easebuzz', 'khalti', 'authorizenet', 'fedapay', 'payhere', 'cinetpay', 'paymentwall', 'whatsapp', 'telegram', 'moolre'];
        $enabled = [];
        ...
        switch ($method) {
            ...
            case 'moolre':
                if (empty($config['username'])) {
                    $errors[] = 'Moolre Username is required';
                }
                if (empty($config['public_key'])) {
                    $errors[] = 'Moolre Public Key is required';
                }
                if (empty($config['account_number'])) {
                    $errors[] = 'Moolre Account Number is required';
                }
                break;
        }
...
if (!function_exists('calculatePlanPricing')) {
    function calculatePlanPricing($plan, $couponCode = null)
    {
...
if (!function_exists('createPlanOrder')) {
    function createPlanOrder($data)
    {
        ...
        $pricing = calculatePlanPricing($plan, $data['coupon_code'] ?? null);
