    ...
    public function processPayment(Request $request)
    {
        ...
        try {
            ...
            if ($result['status'] && $result['data']['status'] === 'success') {
                // Security Check: Verify amount matches plan price
                // Paystack returns amount in minor units (kobo), so divide by 100
                $paidAmount = $result['data']['amount'] / 100;
                $expectedAmount = $validated['billing_cycle'] === 'yearly' ? $plan->yearly_price : $plan->price;
