...
class OrderService
{
    public function __construct(protected MoolreService $moolreService)
    {
    }
    public function createOrder(array $orderData, array $cartItems): Order
    {
        return DB::transaction(function () use ($orderData, $cartItems) {
            ...
            // Define payment methods that require deferred stock reduction (payment verification first)
            $deferredStockPaymentMethods = ['moolre', 'stripe', 'paypal', 'paystack', 'flutterwave', 'razorpay', 'payfast', 'mercadopago', 'cashfree'];
            $shouldReduceStockNow = !in_array($orderData['payment_method'], $deferredStockPaymentMethods);
            ...
            $order = Order::create([
                ...
                'payment_details' => $shouldReduceStockNow ? ['stock_reduced' => true] : [],
            ]);
            ...
            foreach ($cartItems as $cartItem) {
                ...
                if ($product) {
                    ...
                    // Only reduce stock immediately for non-deferred payment methods (COD, WhatsApp, etc.)
                    // For payment gateways like Moolre, stock is reduced after payment confirmation via reduceStockForOrder()
    ...
    public function processPayment(Order $order, string $storeSlug = null): array
    {
        switch ($order->payment_method) {
            ...
            case 'moolre':
                return $this->processMoolrePayment($order, $storeSlug);
            case 'paystack':
    ...
    private function processCashfreePayment(Order $order, string $storeSlug = null): array
    {
        try {
            ...
            $customerId = 'store_customer_' . ($order->customer_id ?: session()->getId());
            $customerDetails->setCustomerId(substr($customerId, 0, 50));
            $customerName = trim($order->customer_first_name . ' ' . $order->customer_last_name);
            $customerDetails->setCustomerName(substr($customerName, 0, 50));
            $customerDetails->setCustomerEmail($order->customer_email);
            $customerPhone = $order->customer_phone ?: '9999999999';
            $customerDetails->setCustomerPhone(strlen($customerPhone) >= 10 ? $customerPhone : '9999999999');

            // Create order meta
            $orderMeta = new \Cashfree\Model\OrderMeta();
            $orderMeta->setReturnUrl(route('store.order-confirmation', [
                'storeSlug' => $storeSlug ?: $storeModel->slug,
                'orderNumber' => $order->order_number
            ]));
            $orderMeta->setNotifyUrl(route('store.cashfree.webhook'));

            // Create order request
            $orderRequest = new \Cashfree\Model\CreateOrderRequest();
            $orderRequest->setOrderId($cashfreeOrderId);
            $orderRequest->setOrderAmount($order->total_amount);
            $orderRequest->setOrderCurrency($cashfreeConfig['currency'] ?? 'INR');
            $orderRequest->setCustomerDetails($customerDetails);
            $orderRequest->setOrderMeta($orderMeta);
            $orderRequest->setOrderNote('Store Order #' . $order->order_number);
            ...
            if (!$apiResponse || !isset($apiResponse[0])) {
                return ['success' => false, 'message' => 'Failed to create Cashfree payment session'];
            }

            $paymentSession = $apiResponse[0];
            ...
            $order->update([
                ...
                'payment_details' => [
                    ...
                    'payment_session_id' => $paymentSession->getPaymentSessionId(),
                ],
            ]);
    ...
    private function processMoolrePayment(Order $order, string $storeSlug = null): array
    {
        try {
            // Get store owner's Moolre settings
            $storeModel = \App\Models\Store::find($order->store_id);
            if (!$storeModel || !$storeModel->user) {
                return ['success' => false, 'message' => 'Store configuration error'];
            }

            // Phase 3: Web POS Redirect Logic
            // Generate the secure payment link
            $linkData = $this->moolreService->generatePaymentLinkForOrder($storeModel, $order);

            // Update order with payment metadata (preserve stock_reduced logic by merging)
            $order->update([
                'payment_gateway' => 'moolre',
                'order_status' => 'pending',
                'payment_status' => 'unpaid',
                'payment_details' => array_merge($order->payment_details ?? [], [
                    'moolre_external_ref' => $linkData['external_ref'],
                    'moolre_reference' => $linkData['reference'],
                    'moolre_auth_url' => $linkData['authorization_url'],
                    'generated_at' => now(),
                ])
            ]);

            // Redirect user to the hosted Moolre Web POS page
            return [
                'success' => true,
                'message' => 'Redirecting to Moolre payment',
                'checkout_url' => $linkData['authorization_url'], // External URL
                'order_id' => $order->id,
                'order_number' => $order->order_number,
            ];

        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Moolre Service Error: ' . $e->getMessage());
            return [
                'success' => false,
                'message' => 'Moolre payment failed: ' . $e->getMessage()
            ];
        }
    }
    ...
    private function processPaystackPayment(Order $order, string $storeSlug = null): array
    {
        try {
            ...
            $paystackReference = 'store_ps_' . $order->id . '_' . time() . '_' . uniqid();

            // Update order with Paystack details
            $order->update([
                'payment_gateway' => 'paystack',
                'payment_transaction_id' => $paystackReference,
                'payment_details' => [
            ...
            return [
                ...
                'paystack_public_key' => $paystackConfig['public_key'],
                'paystack_reference' => $paystackReference,
    ...
    private function processFlutterwavePayment(Order $order, string $storeSlug = null): array
    {
        try {
            ...
            $flutterwaveReference = 'store_fw_' . $order->id . '_' . time() . '_' . uniqid();

            // Update order with Flutterwave details
            $order->update([
                'payment_gateway' => 'flutterwave',
                'payment_transaction_id' => $flutterwaveReference,
                'payment_details' => [
            ...
            return [
                ...
                'flutterwave_public_key' => $flutterwaveConfig['public_key'],
                'flutterwave_reference' => $flutterwaveReference,
     ...
     * This should be called when payment_status changes to 'paid'.
     */
    public function reduceStockForOrder(Order $order): void
    {
        // Skip if stock was already reduced (check in payment_details)
        $paymentDetails = $order->payment_details ?? [];
        if (isset($paymentDetails['stock_reduced']) && $paymentDetails['stock_reduced'] === true) {
            return;
        }
        ...
        foreach ($order->items as $orderItem) {
            ...
            if ($product) {
                if ($product->stock >= $orderItem->quantity) {
                    ...
                    \Log::warning("Insufficient stock for order {$order->order_number} item {$product->name}. Payment received but stock not reduced.");
                }
            }
        }

        // Mark stock as reduced to prevent double-reduction
        $paymentDetails['stock_reduced'] = true;
        $paymentDetails['stock_reduced_at'] = now()->toIso8601String();
    ...
    public function cancelOrder(Order $order): void
    {
        DB::transaction(function () use ($order) {
            // Only restore inventory if stock was actually reduced
            $paymentDetails = $order->payment_details ?? [];
            $stockWasReduced = !empty($paymentDetails['stock_reduced']);
