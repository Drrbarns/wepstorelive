...
Route::prefix('store/{storeSlug}')->middleware('store.status')->group(function () {
    ...
    Route::get('/payfast/success/{orderNumber}', [\App\Http\Controllers\Store\PayFastController::class, 'success'])->name('store.payfast.success');
    Route::get('/mercadopago/success/{orderNumber}', [\App\Http\Controllers\Store\MercadoPagoController::class, 'success'])->name('store.mercadopago.success');
    ...
    Route::get('/moolre/success/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'success'])->name('store.moolre.success');
    Route::get('/paystack/verify', [\App\Http\Controllers\Store\OrderController::class, 'verifyPaystack'])->name('store.paystack.verify');
    Route::get('/flutterwave/verify', [\App\Http\Controllers\Store\OrderController::class, 'verifyFlutterwave'])->name('store.flutterwave.verify');
...
// Moolre store checkout callback (public route)
Route::match(['GET', 'POST'], 'store/{storeSlug}/moolre/callback', [\App\Http\Controllers\Store\MoolreController::class, 'callback'])->name('store.moolre.callback');
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // Moolre plan payment routes
    Route::post('payment/moolre/pay', [\App\Http\Controllers\MoolrePaymentController::class, 'pay'])->name('moolre.payment.pay');
    Route::get('payment/moolre/otp', [\App\Http\Controllers\MoolrePaymentController::class, 'showOtpForm'])->name('moolre.payment.otp');
    Route::post('payment/moolre/verify-otp', [\App\Http\Controllers\MoolrePaymentController::class, 'verifyOtp'])->name('moolre.payment.verify-otp');
    Route::get('payment/moolre/pending', [\App\Http\Controllers\MoolrePaymentController::class, 'pending'])->name('moolre.payment.pending');
    Route::get('payment/moolre/check-status', [\App\Http\Controllers\MoolrePaymentController::class, 'checkStatus'])->name('moolre.payment.check-status');
    Route::post('payment/moolre/confirm', [\App\Http\Controllers\MoolrePaymentController::class, 'confirmPayment'])->name('moolre.payment.confirm');
    Route::get('payment/moolre/success', [\App\Http\Controllers\MoolrePaymentController::class, 'success'])->name('moolre.payment.success');
    Route::get('payment/moolre/return/{reference}', [\App\Http\Controllers\MoolrePaymentController::class, 'returnPage'])->name('moolre.payment.return');
...
// Moolre webhook callback (public route, no auth required)
Route::match(['GET', 'POST'], 'payment/moolre/callback', [\App\Http\Controllers\MoolrePaymentController::class, 'callback'])->name('moolre.payment.callback');
Route::match(['GET', 'POST'], '/moolre/webhook', [\App\Http\Controllers\MoolreWebhookController::class, 'handle'])->name('moolre.global.webhook'); // New Phase 4 Global Webhook

// Store Moolre routes (public, for store checkout flow)
Route::prefix('store/{storeSlug}/moolre')->group(function () {
    Route::get('/return/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'returnPage'])->name('store.moolre.return');
    Route::get('/check-status/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'checkOrderStatus'])->name('store.moolre.check-status');

    Route::get('/pay/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'showPaymentForm'])->name('store.moolre.pay');
    Route::post('/initiate/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'initiatePayment'])->name('store.moolre.initiate');
    Route::post('/verify-otp/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'verifyOtp'])->name('store.moolre.verify-otp');
    Route::post('/confirm/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'confirmPayment'])->name('store.moolre.confirm');
    Route::get('/success/{orderNumber}', [\App\Http\Controllers\Store\MoolreController::class, 'success'])->name('store.moolre.success');
    Route::match(['GET', 'POST'], '/callback', [\App\Http\Controllers\Store\MoolreController::class, 'callback'])->name('store.moolre.callback');
});
